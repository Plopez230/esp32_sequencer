...
uint32_t seq_file_list_page(char *dir_name, uint8_t page, t_seq_file_entry *files)
{
    ...
    file_count = 0;
    max_files = seq_file_count_files(dir_name);
    while (1)
    {
        File entry = dir.openNextFile();
        if (!entry)
...
void seq_file_gui_control(char *dir_name, char *dst, uint32_t dst_size)
{
    ...
    if (total_pages * 6 < total_files)
        total_pages++;
    files_in_page = seq_file_list_page(dst, page, files);
    while (1)
    {
